...
app.service('HttpSvc',[ '$http', function($http){ /* picture, resource api 수정해야함 */
    ...
    return {
        ...
        /* RESOURCE REST API */
        downloadResource: function (_file_idx) {
            return $http({
                url: urlBase + '/file_download/file-idx',
                method: "GET",
                data: {file_idx: _file_idx}
            });
        },

        uploadResource: function (_userfile) {
            return $http({
                headers: {
                    'Content-Type': 'multipart/form-data'
                },
                url: urlBase + '/file_upload',
                method: "POST",
                data: {userfile : _userfile}
            });
        }
